...
exports.createUserToGpt = async (req, res) => {
	...
	try {
		...
		if (!variant || !request_id) {
			return res
				.status(400)
				.json({ message: 'Variant and request_id are required.' })
		}
		...
		if (!request) {
			return res
				.status(404)
				.json({ message: `Request with id ${request_id} not found.` })
		}

		const parent_id = (
			await UserToGpt.findOne({
				where: { request_id },
		...
		const concatenatedText = allVariants
		...
		if (concatenatedText.length > 1000) {
			const fastApiResponse = await axios.post(
				'http://192.168.96.31:8000/novellas/theEnd',
			...
			return res.status(200).json({
				message:
					'The concatenated text exceeded the limit and was sent to /novellas/theEnd.',
				userToGpt: {
					...userToGpt.toJSON(),
		...
		const fastApiResponse = await axios.post(
			'http://192.168.96.31:8000/novellas/g4f',
			{ text: concatenatedText },
			{
				headers: { Authorization: `Bearer ${JWT_SECRET}` },
				auth: {
		...
		if (Array.isArray(options)) {
			...
			return res
				...
				.json({
					message:
						'Invalid response from FastAPI: options field is missing or not an array.',
				})
		}
	} catch (error) {
...
exports.finalizeStory = async (req, res) => {
	...
	if (!request_id || !final_variant) {
		return res
			.status(400)
			.json({ message: 'Request_id and final_variant are required.' })
	}
	...
	try {
		...
		const finalStory =
			allVariants.map(entry => entry.variant).join(' ') + ` ${final_variant}`
